...
def test_rebuild_overwrites_source_and_stays_usable(tmp_path: Path) -> None:
    """建立过读取索引的对象在原位替换后也必须使用新目录和内容。"""
    src = _make_source(tmp_path)
    wad = WAD(str(src))
    assert wad.extract([JSON_PATH], raw=True) == [JSON_OLD]
...
def test_rebuild_keeps_entry_sharing_offset_with_empty_entry(tmp_path: Path) -> None:
    ...
    with BinaryWriter(src) as bw:
        ...
        bw.bytes(pack_entry_v34(2, data_offset, len(blob), len(blob), 0, 0, 0, entry_checksum(blob)))
        bw.bytes(blob)
