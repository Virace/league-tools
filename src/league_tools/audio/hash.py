...
class BinAggregator:
    ...
    @classmethod
    def from_bin_sources(cls, bin_sources: List[Union[StrPath, bytes, BIN]]) -> 'BinAggregator':
        """从多个来源创建聚合器
    ...
    def add_bin(self, bin_obj: BIN) -> None:
        ...
        for group in bin_obj.data:
            for unit in group.bank_units:
                ...
                if category not in self.bank_units_by_category or len(unit.events) > len(self.bank_units_by_category[category].events):
    ...
    def iterate_units(self, sfx_filter: bool = False, vo_filter: bool = False) -> Iterator[Any]:
        ...
        for unit in self.theme_music:
            ...
            category_suffix = unit.category.split('_')[-1] if '_' in unit.category else ""
...
class AudioFiles:
    ...
    @staticmethod
    def load_files(audio_file: Union[StrPath, bytes], get_data=True, hash_table: Optional[List[int]] = None) -> List[WemFile]:
        """从音频文件加载WEM文件列表
        ...
        # 解析BNK文件
            ...
            if data_section:
                ...
                audio_files, data_call = data_section.files, lambda: bnk.objects[b'DATA'].get_files(data_section)
            else:
    ...
    @staticmethod
    def map_events_to_audio(event_hashtable: List[StringHash], audio_file: Union[StrPath, bytes]) -> Dict[str, List[int]]:
        """创建事件到音频ID的映射
        ...
        if not audio_files:
            logger.warning(f"未从音频数据中找到任何音频文件")
            return OrderedDict()
    ...
    def match_sounds(self, event_str: str, event_id: int) -> List[StringHash]:
        ...
        for sound_id, sound in self.bank.sounds.items():
            if sound_id == event_id:
                logger.debug(f'发现资源文件: {event_str}, {sound.source_id}, (Sound直接匹配)')
                res.append(
                    StringHash(
                        string=event_str,
                        hash=sound.source_id
                    )
                )
        return res

    def trace_container(self, event_str: str, event_id: int,
                        visited: Set[int], container_type: str) -> List[StringHash]:
        """追踪容器内容
        ...
        # 获取正确的容器字典
        containers = self.bank.random_containers if container_type == "random" else self.bank.switch_containers

        for container_id, container in containers.items():
            if container_id == event_id:
                logger.debug(f'处理{container_type}容器: ID={container_id}, 事件={event_str}')
                ...
                for child_id in container.child_ids:
                    ...
                    # 2. 递归处理子容器
                    # 随机容器
                    random_results = self.trace_container(event_str, child_id, visited.copy(), "random")
                    res.extend(random_results)

                    # 切换容器
                    switch_results = self.trace_container(event_str, child_id, visited.copy(), "switch")
    ...
    def extract_action_targets(self, event: Any, event_name: str) -> List[StringHash]:
        ...
        for action_id in event.event_ids:
            ...
            if action.action_type == 1027:
                logger.debug(f"处理播放动作: ID={action_id}, 引用ID={action.id_ext}, 事件={event_name}")
                ...
                visited = set()
                # 随机容器
                random_results = self.trace_container(event_name, action.id_ext, visited, "random")
                res.extend(random_results)

                # 切换容器
                switch_results = self.trace_container(event_name, action.id_ext, visited, "switch")
                res.extend(switch_results)
...
def map_events_to_audio_ids(bank: WwiserBank, bin_events: List[StringHash]) -> List[StringHash]:
    """从事件BNK中提取事件与音频ID的映射
...
class FileProcessor:
    ...
    def combine_files(audio_files_list: List[List[WemFile]]) -> List[WemFile]:
        ...
        for files in audio_files_list:
            for file in files:
                ...
                if file.id not in result_dict or (not result_dict[file.id].data and file.data):
    ...
    @staticmethod
    def load_from_paths(bank_paths: List[str], raw_files: List[bytes],
                        cache_dir: StrPath, get_data: bool = False) -> List[WemFile]:
        """从多个bank_path中提取音频文件
...
class Extractor:
    ...
    def save_audio(self, audio_hashtable: Dict[str, List[int]], audio_file: Union[StrPath, bytes],
                   output_dir: StrPath, output_format: str = 'wem', keep_wem: bool = False) -> Dict[str, List[str]]:
        """保存音频文件到输出目录
        ...
        if convert_needed and not self.vgmstream_cli:
            logger.warning(f"未提供vgmstream_cli路径，无法转换为{output_format}格式，将使用原始wem格式")
            convert_needed = False
            output_format = 'wem'

        # 或者检查vgmstream是否存在
        if convert_needed and self.vgmstream_cli and not os.path.exists(self.vgmstream_cli):
            logger.warning(f"vgmstream_cli路径无效: {self.vgmstream_cli}，无法转换音频格式")
            convert_needed = False
            output_format = 'wem'
            ...
            for audio_id in audio_ids:
                ...
                try:
                    ...
                    if convert_needed:
                        ...
                        # 使用vgmstream转换
                        os.system(f'"{self.vgmstream_cli}" -o "{output_file}" "{wem_path}"')
        ...
        logger.info(f"音频提取完成，共处理 {len(result)} 个事件，输出格式: {output_format}")
        return result
...
class Context:
    ...
    def __init__(self, bin_file_path: Optional[StrPath] = None, 
    ...
    def process_unit(self, bank_unit: Any, category: str) -> Dict[str, List[int]]:
        ...
        try:
            ...
            if audio_file_path:
                ...
                # 构建音频哈希表
                audio_hashtable = AudioFiles.map_events_to_audio(event_audio_mapping, audio_data)

                if audio_hashtable:
                    logger.info(f"成功构建音频哈希表: {category}, 事件数: {len(audio_hashtable)}")
...
class Processor:
    ...
    def collect_mappings_from_aggregator(self, aggregator: BinAggregator) -> Dict[str, Dict[str, List[int]]]:
        """从BIN聚合器收集映射关系
        ...
        # 处理所有bank_units
        total_units = len(aggregator.bank_units_by_category) + len(aggregator.theme_music)
        processed_count = 0
    ...
    def collect_mappings(self) -> Dict[str, Dict[str, List[int]]]:
        ...
        # 按类别处理
        processed_count = 0
        total_bank_units = sum(len(group.bank_units) for group in self.context.bin_obj.data)

        for audio_group in self.context.bin_obj.data:
            for bank_unit in audio_group.bank_units:
                processed_count += 1
                logger.info(f"处理进度: {processed_count}/{total_bank_units} - 类别: {bank_unit.category}")
                ...
                # 处理资源集合单元
                audio_hashtable = self.bank_processor.process_unit(bank_unit, bank_unit.category)
    ...
    def extract_all(self, vgmstream_cli: Optional[StrPath] = None,
                    output_format: str = 'wem',
                    keep_wem: bool = False) -> Dict[str, Dict[str, List[str]]]:
        """提取并处理所有音频文件
        ...
        for category, hashtable in audio_mappings.items():
            ...
            # 确定资源类型和对应的WAD
            resource_type = determine_resource_type(category)
            wad = self.context.region_wad if resource_type == "VO" else self.context.base_wad
            ...
            if extracted_files:
                ...
                logger.info(f"类别 {category} 提取完成，共 {len(extracted_files)} 个事件")
